...
class GaussianProcessVisualizer(mll.GaussianProcessLearner):
    ...
    `GaussianProcessVisualizer` extends of `GaussianProcessLearner`, but is
    designed not to be used as a learner, but to instead post process a
    `GaussianProcessLearner` archive file and produce visualizations. This class
    fixes the Gaussian process hyperparameters to what was last found during the
    run.

    If a training archive was provided at the start of the optimization using
    `training_filename` and that training archive was generated by a Gaussian
    ...
    def __init__(self, filename, **kwargs):
        ...
        if 'file_type' in kwargs:
            msg = (
                "The file_type argument has been deprecated and will be "
                "ignored."
            )
            warnings.warn(msg)

        super(GaussianProcessVisualizer, self).__init__(
    ...
    def plot_hyperparameters_vs_fit(self, parameter_subset=None):
        ...
        one was provided with `training_filename` and the training archive was
...
class NeuralNetVisualizer(mll.NeuralNetLearner):
    ...
    `NeuralNetVisualizer` extends of `NeuralNetLearner`, but is designed not to
    be used as a learner, but to instead post process a `NeuralNetLearner`
    archive file and produce visualizations.
    ...
    def __init__(self, filename, file_type = None, **kwargs):
        ...
        super(NeuralNetVisualizer, self).__init__(
            training_filename = filename,
            update_hyperparameters = False,
            learner_archive_filename=None,
            **kwargs,
